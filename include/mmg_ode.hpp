...
public:
    ...
    void operator() ( const std::array<double,3> &x , std::array<double,3> &dxdt , double t )
    {
        ...
        // if (J!=0)
        // {
        // u_R = u* (1 - w_P)* pm.epsilon* sqrt(pm.eta * pow(1.0 + pm.kappa * (sqrt(1.0 + 8.0 * K_T / (2 * acos(0.0) * pow(J, 2))) - 1),2+ (1 - pm.eta)));
        // }  
        
        // double U_R = sqrt(pow(u_R, 2) + pow(v_R, 2)); // Resultant inflow velocity to rudder
        
        // double alpha_R = pm.delta - atan2(v_R, u_R); // Effective inflow angle to rudder
        ...
        // double X_R = -(1 - pm.t_R) * F_N * sin(pm.delta); // Surge force around midship by steering
        ...
        //     )
        // );
        // Y_R = -(1 + pm.a_H) * F_N * cos(pm.delta); // Lateral force around midship by steering
        ...
        //     )
        // );
        // double N_R = -(pm.x_R + pm.a_H * pm.x_H) * F_N * cos(pm.delta); // Yaw moment around midship by steering
        ...
        dxdt[0] = pm.m/(10.0*pm.m) + x[0]/(8.0*pm.m);
        dxdt[1] = pm.m/(4.0*pm.m) + x[1]/(4.0*pm.m);
        dxdt[2] = pm.m/(2.0*pm.m) + x[2]/(1.0*pm.m);
    }
};
#endif //MMG_ODE_HPP